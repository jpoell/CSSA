...
#'
#' @note The simulator is currently not set up to perform resistance screen
#'   simulation. A simple alteration to accommodate this is to add a growth rate
#'   modifier in the formula. This "grm" will then have to be present within
#'   brackets for the successfully knocked out cells and be added to the formula
#'   for failed knock out cells. It makes sense to default to grm = 1, and then
#'   my straight lethal variable d will be centered around 0 instead of 1. For
#'   now that changes a bit too much of my other analyses, but can be
#'   incorporated if deemed useful.
#'
#' @author Jos B. Poell
#'
#' @examples
  ...
  for (t in seq_along(a)) {
    ...
    if (missing(e) || length(e) == 1) {
      ...
      cellpool <- mapply(function(stud, dud, d, e) {
        kocells <- round(stud*2^((grm^st+d+e)*a[t]))
        nkocells <- round(dud*2^(grm^st*a[t]))
        return(list(kocells = kocells, nkocells = nkocells))
